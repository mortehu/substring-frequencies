      ...
      else
        {
          switch (m)
            ...
            case MODE_TAG:
              ...
              if (ch == '>' || isspace (ch))
                {
                  if (has_context ("<script"))
                    tag = TAG_SCRIPT;
                  else if (has_context ("<style"))
                    tag = TAG_STYLE;
                }
              ...
              else if (ch == '[' && has_context ("<![CDATA"))
                next_mode = MODE_CDATA;
              else if (ch == '-' && has_context ("<!-"))
            ...
            case MODE_DOCUMENT_BOUNDARY: assert ("!Got MODE_DOCUMENT_BOUNDARY");
            case MODE_INVALID: assert ("!Got MODE_INVALID");
            }
        }

      switch (m)
        {
        case MODE_HEADER: class = 1; break;
        case MODE_CDATA: class = 2; break;
        case MODE_COMMENT: class = 3; break;
        case MODE_SCRIPT: class = 4; break;
        case MODE_TAG: class = 5; break;
        case MODE_TAG_ATTRIBUTES: class = 6; break;
        case MODE_TEXT: class = (tag == TAG_STYLE) ? 7 : 0; break;
        default: class = 0;
        }
